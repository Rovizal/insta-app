...
const CreatePostModal = ({ handleAddPost, closeModal, isOpen }) => {
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 transition-opacity duration-300 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}>
      ...
      <div
        className={`bg-white p-6 rounded-lg shadow-lg w-full max-w-xl relative transform transition-all duration-300 ease-in-out 
      max-h-screen overflow-auto ${showModal ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
      >
